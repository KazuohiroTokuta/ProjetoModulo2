					...
					<tbody>
						...
						<jstl:forEach items="${listaPedidos}" var="a">
							<tr>
								...
								<td>${a.dt_pedido}</td>
								<td>${a.forma_pgmto}</td>
								<td>${a.passageiro.CPF}</td>
								<td>
									<div class="d-flex">
										<a href="pedido-edit?num_pedido=${a.num_pedido}" class="mx-1"
											title="Editar"> <i class="ri-file-edit-line"></i>
										</a> <a href="pedido-delete?num_pedido=${a.num_pedido}"
											class="mx-1" title="Cancelar"
											onclick="return confirm('Deseja excluir o pedido ${a.num_pedido}.')">
